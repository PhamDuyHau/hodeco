 ...
 * lost-password, and comment forms based on settings.
 *
...
final class FormProtection {
	 ...
	 * @param array<string, bool>      $forms    Form flags from settings.
	 */
	public function __construct( CaptchaProviderInterface $provider, array $forms ) {
	...
	private function initLoginPageForms(): void {
		$hasLoginForms = ! empty( $this->forms[ Recaptcha::KEY_FORM_LOGIN ] )
			|| ! empty( $this->forms[ Recaptcha::KEY_FORM_REGISTER ] )
			|| ! empty( $this->forms[ Recaptcha::KEY_FORM_LOST_PASSWORD ] );
	...
	private function initCommentForm(): void {
		if ( empty( $this->forms[ Recaptcha::KEY_FORM_COMMENT ] ) ) {
			return;
		}
	...
	public function verifyComment( array $commentData ): array {
		...
		if ( is_wp_error( $result ) ) {
			wp_die(
				...
				esc_html__( 'CAPTCHA Verification Failed', HDA_TEXTDOMAIN ),
	...
	private function verifyOrError( string $errorCode ): true|\WP_Error {
		...
		if ( empty( $token ) ) {
			return new \WP_Error(
				$errorCode,
				__( '<strong>Error:</strong> Please complete the CAPTCHA verification.', HDA_TEXTDOMAIN )
			);
		}
		...
		if ( ! $this->provider->verifyToken( $token ) ) {
			return new \WP_Error(
				$errorCode,
				__( '<strong>Error:</strong> CAPTCHA verification failed. Please try again.', HDA_TEXTDOMAIN )
			);
		}
