...
trait AjaxHandlerTrait {
	 ...
	 */
	private const string NONCE_ACTION = 'custom_sorting_';
	...
	private function refreshPostTypeOrder(): void {
		...
		foreach ( $this->orderPostType as $postType ) {
			$result = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT COUNT(*) AS cnt, MAX(menu_order) AS max_order, MIN(menu_order) AS min_order
					FROM {$wpdb->posts}
			...
			if ( (int) $result->cnt === (int) $result->max_order && (int) $result->min_order === 1 ) {
				continue;
			}
	...
	private function refreshTaxonomyOrder(): void {
		...
		foreach ( $this->orderTaxonomy as $taxonomy ) {
			$result = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT COUNT(*) AS cnt, MAX(term_order) AS max_order, MIN(term_order) AS min_order
	...
	public function updateMenuOrderAjax(): void {
		...
		if ( ! check_ajax_referer( self::NONCE_ACTION . get_current_user_id(), 'nonce', false ) ) {
	...
	private function updatePostTypeOptions( \wpdb $wpdb ): void {
		...
		foreach ( $this->orderPostType as $postType ) {
			...
			$result = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT COUNT(*) AS cnt, MAX(menu_order) AS max_order
					FROM {$wpdb->posts}
					WHERE post_type = %s AND post_status IN ('publish', 'pending', 'draft', 'private', 'future')",
					$postType
				)
			...
			if ( ! $result || (int) $result->cnt === 0 || (int) $result->cnt === (int) $result->max_order ) {
				continue;
			}
	...
	private function updateTaxonomyOptions( \wpdb $wpdb ): void {
		...
		foreach ( $this->orderTaxonomy as $taxonomy ) {
			...
			$result = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT COUNT(*) AS cnt, MAX(term_order) AS max_order
					FROM {$wpdb->terms} AS t
					INNER JOIN {$wpdb->term_taxonomy} AS tt ON t.term_id = tt.term_id
					WHERE tt.taxonomy = %s",
					$taxonomy
	...
	public function resetAll(): void {
		...
		Helper::removeOption( self::OPTION_NAME );
