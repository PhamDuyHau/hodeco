...
namespace HDAddons\Security\ServerConfig;
...
final class ServerConfig {
	 ...
	 */
	public const string MARKER        = 'HDA';
	public const string XMLRPC_MARKER = 'HDA-XMLRPC';
	public const string OPML_MARKER   = 'HDA-OPML';
	...
	public static function getServerLabel(): string {
		return match ( self::detectServerType() ) {
			...
			'litespeed' => 'LiteSpeed',
			'nginx'     => 'Nginx',
			default     => __( 'Unknown', HDA_TEXTDOMAIN ),
		};
	}
	...
	public static function addHtaccessBlock( string $marker = self::MARKER, string $templateName = 'htaccess.tpl' ): bool|string {
		...
		if ( ! is_writable( $dir ) ) {
			return sprintf(
				...
				__( 'Directory is not writable: %s', HDA_TEXTDOMAIN ),
				$dir
			);
		}
		...
		if ( empty( $template ) ) {
			...
			return sprintf(
				...
				__( 'Failed to load template: %s', HDA_TEXTDOMAIN ),
				$templateName
			);
		}
		...
		if ( false === $result ) {
			return __( 'Failed to write to .htaccess file.', HDA_TEXTDOMAIN );
		}
	...
	public static function removeHtaccessBlock( string $marker = self::MARKER ): bool|string {
		...
		if ( false === $content ) {
			...
			return __( 'Failed to read .htaccess file.', HDA_TEXTDOMAIN );
		}
		...
		if ( false === $result ) {
			return __( 'Failed to update .htaccess file.', HDA_TEXTDOMAIN );
		}
	...
	public static function addNginxBlock( string $marker = self::MARKER, string $templateName = 'nginx.conf' ): bool|string {
		...
		if ( false === $result ) {
			return sprintf(
				...
				__( 'Failed to write Nginx config file: %s', HDA_TEXTDOMAIN ),
				$confPath
			);
		}
	...
	public static function removeNginxBlock( string $marker = self::MARKER ): bool|string {
		...
		if ( ! is_writable( $confPath ) ) {
			return sprintf(
				/* translators: %s: file path */
				__( 'File is not writable: %s', HDA_TEXTDOMAIN ),
				$confPath
			);
		}
		...
		if ( false === $content ) {
			return __( 'Failed to read Nginx config file.', HDA_TEXTDOMAIN );
		}
		...
		if ( '' === trim( $newContent ) ) {
			...
			if ( file_exists( $confPath ) ) {
				return sprintf(
					/* translators: %s: file path */
					__( 'Failed to delete Nginx config file: %s', HDA_TEXTDOMAIN ),
					$confPath
				);
			}
		...
		if ( false === $result ) {
			return __( 'Failed to update Nginx config file.', HDA_TEXTDOMAIN );
		}
	...
	public static function addBlock(
		...
		return __( 'Unsupported server type. Only Apache and Nginx are supported.', HDA_TEXTDOMAIN );
	}
	...
	public static function removeBlock( string $marker = self::MARKER ): bool|string {
		...
		return __( 'Unsupported server type.', HDA_TEXTDOMAIN );
	}
	...
	public static function lockFile( string $path ): bool|string {
		if ( ! file_exists( $path ) ) {
			return sprintf(
				/* translators: %s: file path */
				__( 'File not found: %s', HDA_TEXTDOMAIN ),
				$path
			);
		}
		...
		if ( ! @chmod( $path, self::PERM_LOCKED ) ) {
			return sprintf(
				/* translators: %s: file path */
				__( 'Failed to lock file: %s', HDA_TEXTDOMAIN ),
				$path
			);
		}
	...
	public static function unlockFile( string $path ): bool|string {
		...
		if ( ! @chmod( $path, self::PERM_UNLOCKED ) ) {
			return sprintf(
				/* translators: %s: file path */
				__( 'Failed to unlock file: %s', HDA_TEXTDOMAIN ),
				$path
			);
		}
