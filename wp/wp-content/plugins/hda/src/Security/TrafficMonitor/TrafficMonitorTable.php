...
final class TrafficMonitorTable extends \WP_List_Table {
	...
	public function __construct() {
		parent::__construct(
			[
				'singular' => __( 'Traffic Log', HDA_TEXTDOMAIN ),
				'plural'   => __( 'Traffic Logs', HDA_TEXTDOMAIN ),
				'ajax'     => false,
			]
		);
	}
	...
	public function get_columns(): array {
		return [
			...
			'ip'          => __( 'IP Address', HDA_TEXTDOMAIN ),
			'uri'         => __( 'URI', HDA_TEXTDOMAIN ),
			'method'      => __( 'Method', HDA_TEXTDOMAIN ),
			'action'      => __( 'Action', HDA_TEXTDOMAIN ),
			'attack_type' => __( 'Attack Type', HDA_TEXTDOMAIN ),
			'severity'    => __( 'Severity', HDA_TEXTDOMAIN ),
			'created_at'  => __( 'Time', HDA_TEXTDOMAIN ),
		];
	}
	...
	public function prepare_items(): void {
		...
		$filterAction = isset( $_REQUEST['filter_action'] ) ? sanitize_key( $_REQUEST['filter_action'] ) : '';
		$filterType   = isset( $_REQUEST['filter_type'] ) ? sanitize_key( $_REQUEST['filter_type'] ) : '';
		$filterIp     = isset( $_REQUEST['filter_ip'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['filter_ip'] ) ) : '';
	...
	protected function extra_tablenav( $which ): void {
		...
		$currentAction = isset( $_REQUEST['filter_action'] ) ? sanitize_key( $_REQUEST['filter_action'] ) : '';
		$currentType   = isset( $_REQUEST['filter_type'] ) ? sanitize_key( $_REQUEST['filter_type'] ) : '';

		echo '<div class="alignleft actions">';

		// Action filter.
		$actions = [ '' => __( 'All Actions', HDA_TEXTDOMAIN ), 'blocked' => 'Blocked', 'logged' => 'Logged', 'allowed' => 'Allowed' ];
		...
		submit_button( __( 'Filter', HDA_TEXTDOMAIN ), '', 'filter_submit', false );

		// Clear All button (only when items exist).
		if ( $this->totalItems > 0 ) {
			echo '&nbsp;';
			wp_nonce_field( TrafficMonitorAdmin::CLEAR_ALL_NONCE, '_wpnonce_clear', false );
			submit_button( __( 'Clear All', HDA_TEXTDOMAIN ), 'delete', 'clear_all_logs', false );
		}
	...
	public function column_ip( $item ): string {
		...
		$filterUrl = add_query_arg(
			[
				'page'      => TrafficMonitorAdmin::MENU_SLUG,
				'filter_ip' => $item['ip'],
			],
			admin_url( 'admin.php' )
		);
	...
	public function column_created_at( $item ): string {
		...
		return sprintf(
			...
			esc_html( human_time_diff( $timestamp, time() ) ) . ' ' . esc_html__( 'ago', HDA_TEXTDOMAIN )
		);
	}
	...
	public function no_items(): void {
		esc_html_e( 'No traffic logs recorded yet. Enable the Firewall module to start logging.', HDA_TEXTDOMAIN );
	}
}
