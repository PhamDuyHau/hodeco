<?php
/**
 * Rate Limiter — transient-based request rate limiting.
 *
 * Tracks request counts per IP + endpoint type using WordPress transients.
 * When an IP exceeds the threshold, the request is flagged for blocking.
 *
 * Uses transients instead of custom DB tables for:
 * - Automatic expiration (no cleanup cron needed)
 * - Object cache compatibility (Redis/Memcached)
 * - Lower write overhead than DB inserts
 *
...
final class RateLimiter {
	...
	/**
	 * Transient key prefix for rate counters.
	 */
	private const string PREFIX = 'hda_fw_rl_';
	...
	private const array DEFAULTS = [
		'global'  => [ 'limit' => 300, 'window' => 60 ],    // 300 req/min
		...
		'xmlrpc'  => [ 'limit' => 5,   'window' => 60 ],    // 5 req/min
		'rest'    => [ 'limit' => 120, 'window' => 60 ],     // 120 req/min
		'ajax'    => [ 'limit' => 120, 'window' => 60 ],     // 120 req/min
	];
	...
	/**
	 * Build a transient key for an IP + endpoint.
	 *
	 * @param string $ip       Client IP.
	 * @param string $endpoint Endpoint type.
	 *
	 * @return string Transient key (max 172 chars → safe for WP).
	 */
	private function key( string $ip, string $endpoint ): string {
		// Hash the IP to keep key length consistent and safe.
		$hash = substr( md5( $ip ), 0, 12 );

		return self::PREFIX . $endpoint . '_' . $hash;
	}
}
