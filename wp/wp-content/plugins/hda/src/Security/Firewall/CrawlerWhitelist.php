...
final class CrawlerWhitelist {
	 ...
	 */
	private const string CACHE_GOOGLE     = 'hda_fw_googlebot_ips';
	private const string CACHE_BING       = 'hda_fw_bingbot_ips';
	private const string CACHE_CLOUDFLARE = 'hda_fw_cloudflare_ips';
	private const string CACHE_VERIFIED   = 'hda_fw_verified_';
	 ...
	 */
	private const array CLOUDFLARE_URLS = [
	...
	public function isVerifiedCrawler( string $ip, string $userAgent ): ?string {
		...
		$cacheKey = self::CACHE_VERIFIED . md5( $ip );
		$cached   = get_transient( $cacheKey );
		if ( false !== $cached ) {
			return $cached ?: null; // Empty string = not a crawler.
		}
		...
		foreach ( self::SOURCES as $name => $source ) {
			...
			if ( ! empty( $ranges ) && Helper::ipMatchesAny( $ip, $ranges ) ) {
				// Cache positive result for 1 hour (IPs don't change often).
				set_transient( $cacheKey, $name, HOUR_IN_SECONDS );
		...
		if ( ! empty( $cfRanges ) && Helper::ipMatchesAny( $ip, $cfRanges ) ) {
			set_transient( $cacheKey, 'cloudflare', HOUR_IN_SECONDS );

			return 'cloudflare';
		}

		// Not a verified crawler — cache negative result to avoid re-checking.
		set_transient( $cacheKey, '', 10 * MINUTE_IN_SECONDS );
	 ...
	 *
	 * Note: Verified IP transient cleanup uses direct SQL because keys contain
	 * MD5 hashes that can't be enumerated via delete_transient(). This only
	 * clears DB-backed transients — sites using external object cache
	 * (Redis/Memcached) will have these entries expire naturally via TTL.
	 *
	 * @return void
	 */
	public static function clearCache(): void {
		...
		// Clear verified IP caches (DB-backed transients only; see docblock).
		global $wpdb;
		$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_hda_fw_verified_%'" );
		$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_timeout_hda_fw_verified_%'" );
	}
}
