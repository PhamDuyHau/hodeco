...
final class Monitor404Table extends \WP_List_Table {
	...
	public function __construct() {
		parent::__construct(
			[
				'singular' => __( '404 Log', HDA_TEXTDOMAIN ),
				'plural'   => __( '404 Logs', HDA_TEXTDOMAIN ),
				'ajax'     => false,
			]
		);
	}
	...
	public function get_columns(): array {
		return [
			...
			'url'        => __( 'URL', HDA_TEXTDOMAIN ),
			'referer'    => __( 'Referer', HDA_TEXTDOMAIN ),
			'hit_count'  => __( 'Hits', HDA_TEXTDOMAIN ),
			'ip_address' => __( 'IP Address', HDA_TEXTDOMAIN ),
			'updated_at' => __( 'Last Hit', HDA_TEXTDOMAIN ),
		];
	}
	...
	protected function extra_tablenav( $which ): void {
		...
		if ( $this->totalItems > 0 ) {
			...
			wp_nonce_field( Monitor404Admin::CLEAR_ALL_NONCE, '_hda_clear_nonce', false );
	...
	public function column_url( $item ): string {
		...
		$actions['delete'] = sprintf(
			...
			wp_nonce_url(
				add_query_arg(
					[
						'page'    => Monitor404Admin::MENU_SLUG,
						'action'  => 'delete',
						'log_ids' => [ $item['id'] ],
					],
					admin_url( 'admin.php' )
			...
			esc_js( __( 'Are you sure?', HDA_TEXTDOMAIN ) ),
			__( 'Delete', HDA_TEXTDOMAIN )
		);
	...
	public function column_referer( array $item ): string {
		...
		if ( empty( $referer ) ) {
			return '<em>' . esc_html__( 'Direct', HDA_TEXTDOMAIN ) . '</em>';
		}
	...
	public function no_items(): void {
		esc_html_e( 'No 404 errors have been logged yet.', HDA_TEXTDOMAIN );
	}
}
