...
trait Options {
	 ...
	 */
	private const OPTION_STORAGE_POST_TYPE = 'hda_storage';
	...
	public static function registerStoragePostType(): void {
		if ( post_type_exists( self::OPTION_STORAGE_POST_TYPE ) ) {
			return;
		}

		register_post_type(
			self::OPTION_STORAGE_POST_TYPE,
			[
	...
	public static function getStoredOption( string $optionKey ): ?array {
		...
		if ( $postId > 0 ) {
			...
			if ( $post && $post->post_type === self::OPTION_STORAGE_POST_TYPE ) {
	...
	public static function updateStoredOption( string $optionKey, string $content, string $contentType = 'text' ): int|\WP_Error {
		if ( ! $optionKey ) {
			return new \WP_Error( 'invalid_key', __( 'Option key is required.', HDA_TEXTDOMAIN ) );
		}
		...
		// Remove magic quotes slashes from $_POST data.
		...
		if ( strlen( $content ) > 1048576 ) {
			return new \WP_Error(
				'content_too_large',
				__( 'Content is too large. Maximum size is 1MB.', HDA_TEXTDOMAIN )
			);
		}
		...
		$postData = [
			'post_type'    => self::OPTION_STORAGE_POST_TYPE,
			'post_status'  => 'publish',
	...
	private static function sanitizeStoredContent( string $content, string $contentType ): string {
		return match ( $contentType ) {
			...
			'html', 'text/html' => self::extractJS( $content ),
			default => $content,
		};
	}
}
