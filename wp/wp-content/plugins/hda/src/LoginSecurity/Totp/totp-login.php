...
<div id="login_error" class="notice notice-error">
	<p><strong><?php esc_html_e( 'Error:', HDA_TEXTDOMAIN ); ?></strong> <?php echo esc_html( $args['error'] ); ?></p>
</div>
<?php endif ?>
...
<form name="totp_validate_form" id="loginform" action="<?php echo esc_url( $args['action'] ?? '' ); ?>" method="post">
	...
	<p class="otp-prompt">
		<?php esc_html_e( 'Enter the 6-digit code from your authenticator app (Google Authenticator, Authy, Microsoft Authenticator, etc.)', HDA_TEXTDOMAIN ); ?>
	</p>
	<p class="auth extra">
		<label for="authcode">
			<?php esc_html_e( 'Authentication Code:', HDA_TEXTDOMAIN ); ?>
		</label>
		<input
			required
			autofocus
	...
	<p class="otp-help">
		<?php esc_html_e( "Can't access your authenticator app? Contact the site administrator.", HDA_TEXTDOMAIN ); ?>
	</p>
	<?php submit_button( __( 'Verify', HDA_TEXTDOMAIN ) ); ?>
</form>
