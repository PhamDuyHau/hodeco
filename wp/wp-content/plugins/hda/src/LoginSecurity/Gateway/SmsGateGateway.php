...
class SmsGateGateway extends AbstractGateway {

	/**
	 * Default cloud API endpoint.
	 */
	private const string DEFAULT_API_URL = 'https://api.sms-gate.app';

	/**
	 * API path for sending messages.
	 */
	private const string SEND_PATH = '/3rdparty/v1/message';
	...
	public function getLabel(): string {
		return __( 'SMSGate', HDA_TEXTDOMAIN );
	}
	...
	public function validateConfig(): bool {
		...
		if ( empty( $username ) ) {
			$this->setError( __( 'SMSGate username is required.', HDA_TEXTDOMAIN ) );

			return false;
		}

		if ( empty( $password ) ) {
			$this->setError( __( 'SMSGate password is required.', HDA_TEXTDOMAIN ) );
	...
	public function send( string $to, string $otp ): bool {
		...
		if ( ! $result['ok'] ) {
			if ( $result['body'] !== null ) {
				$this->setError( $result['body']['message'] ?? __( 'Unknown SMSGate error', HDA_TEXTDOMAIN ) );
			} elseif ( empty( $this->lastError ) ) {
				$this->setError( __( 'Unknown SMSGate error', HDA_TEXTDOMAIN ) );
			}
		...
		if ( $body === null ) {
			$this->setError( __( 'SMSGate returned an invalid response.', HDA_TEXTDOMAIN ) );
		...
		if ( ! in_array( $state, [ 'Pending', 'Sent', 'Delivered' ], true ) ) {
			$this->setError(
				...
				sprintf( __( 'SMSGate message state: %s', HDA_TEXTDOMAIN ), $state ?: 'unknown' )
			);
