...
class TelegramGateway extends AbstractGateway {
	...
	public function getLabel(): string {
		return __( 'Telegram', HDA_TEXTDOMAIN );
	}
	...
	public function validateConfig(): bool {
		...
		if ( empty( $bot_token ) ) {
			$this->setError( __( 'Telegram Bot Token is required.', HDA_TEXTDOMAIN ) );
	...
	public function send( string $to, string $otp ): bool {
		...
		if ( empty( $to ) ) {
			$this->setError( __( 'User has no Telegram Chat ID configured.', HDA_TEXTDOMAIN ) );
		...
		if ( empty( $body['ok'] ) ) {
			$this->setError( $body['description'] ?? __( 'Unknown Telegram error', HDA_TEXTDOMAIN ) );
