...
class DiscordGateway extends AbstractGateway {
	...
	public function getLabel(): string {
		return __( 'Discord', HDA_TEXTDOMAIN );
	}
	...
	public function validateConfig(): bool {
		...
		if ( empty( $bot_token ) ) {
			$this->setError( __( 'Discord Bot Token is required.', HDA_TEXTDOMAIN ) );
	...
	public function send( string $to, string $otp ): bool {
		...
		if ( empty( $to ) ) {
			$this->setError( __( 'User has no Discord User ID configured.', HDA_TEXTDOMAIN ) );
	...
	private function createDMChannel( string $bot_token, string $user_id ): ?string {
		...
		if ( ! $result['ok'] ) {
			if ( empty( $this->lastError ) ) {
				$msg = ( $result['body'] !== null )
					? ( $result['body']['message'] ?? __( 'Failed to create Discord DM channel', HDA_TEXTDOMAIN ) )
					: __( 'Failed to create Discord DM channel', HDA_TEXTDOMAIN );
				$this->setError( $msg );
			}
		...
		if ( empty( $body['id'] ) ) {
			$this->setError( __( 'Discord DM channel ID missing from response.', HDA_TEXTDOMAIN ) );
	...
	private function sendMessage( string $bot_token, string $channel_id, string $message ): bool {
		...
		if ( ! $result['ok'] ) {
			if ( empty( $this->lastError ) ) {
				$msg = ( $result['body'] !== null )
					? ( $result['body']['message'] ?? __( 'Failed to send Discord message', HDA_TEXTDOMAIN ) )
					: __( 'Failed to send Discord message', HDA_TEXTDOMAIN );
				$this->setError( $msg );
			}
