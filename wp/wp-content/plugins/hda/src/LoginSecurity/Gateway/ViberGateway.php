...
class ViberGateway extends AbstractGateway {

	/**
	 * Viber Bot API endpoint for sending messages.
	 */
	private const string API_URL = 'https://chatapi.viber.com/pa/send_message';
	...
	public function getLabel(): string {
		return __( 'Viber', HDA_TEXTDOMAIN );
	}
	...
	public function validateConfig(): bool {
		...
		if ( empty( $auth_token ) ) {
			$this->setError( __( 'Viber Bot Auth Token is required.', HDA_TEXTDOMAIN ) );
	...
	public function send( string $to, string $otp ): bool {
		...
		if ( empty( $to ) ) {
			$this->setError( __( 'User has no Viber User ID configured.', HDA_TEXTDOMAIN ) );
		...
		if ( $result['body'] === null ) {
			if ( empty( $this->lastError ) ) {
				$this->setError( __( 'Viber returned an invalid response.', HDA_TEXTDOMAIN ) );
			}
		...
		if ( ( $body['status'] ?? -1 ) !== 0 ) {
			$this->setError( $body['status_message'] ?? __( 'Unknown Viber error', HDA_TEXTDOMAIN ) );
