...
class LineGateway extends AbstractGateway {

	/**
	 * LINE Messaging API push message endpoint.
	 */
	private const string API_URL = 'https://api.line.me/v2/bot/message/push';
	...
	public function validateConfig(): bool {
		...
		if ( empty( $channel_access_token ) ) {
			$this->setError( __( 'LINE Channel Access Token is required.', HDA_TEXTDOMAIN ) );
	...
	public function send( string $to, string $otp ): bool {
		...
		if ( empty( $to ) ) {
			$this->setError( __( 'User has no LINE User ID configured.', HDA_TEXTDOMAIN ) );
		...
		if ( ! $result['ok'] ) {
			if ( $result['body'] !== null ) {
				$this->setError( $result['body']['message'] ?? __( 'Unknown LINE error', HDA_TEXTDOMAIN ) );
			} elseif ( empty( $this->lastError ) ) {
				$this->setError( __( 'Unknown LINE error', HDA_TEXTDOMAIN ) );
			}
