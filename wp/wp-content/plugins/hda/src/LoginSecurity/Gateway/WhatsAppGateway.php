...
class WhatsAppGateway extends AbstractGateway {
	 ...
	 */
	private const string API_URL = 'https://graph.facebook.com/v21.0/%s/messages';
	...
	public function getLabel(): string {
		return __( 'WhatsApp (Free)', HDA_TEXTDOMAIN );
	}
	...
	public function validateConfig(): bool {
		...
		if ( empty( $phone_number_id ) ) {
			$this->setError( __( 'WhatsApp Phone Number ID is required.', HDA_TEXTDOMAIN ) );

			return false;
		}

		if ( empty( $access_token ) ) {
			$this->setError( __( 'WhatsApp Access Token is required.', HDA_TEXTDOMAIN ) );
	...
	public function send( string $to, string $otp ): bool {
		...
		if ( ! $result['ok'] ) {
			if ( $result['body'] !== null ) {
				$this->setError( $result['body']['error']['message'] ?? __( 'Unknown WhatsApp error', HDA_TEXTDOMAIN ) );
			} elseif ( empty( $this->lastError ) ) {
				$this->setError( __( 'Unknown WhatsApp error', HDA_TEXTDOMAIN ) );
			}
		...
		if ( empty( $body['messages'][0]['id'] ) ) {
			$this->setError( __( 'WhatsApp message not sent', HDA_TEXTDOMAIN ) );
