...
class LoginIllegalUsers {
	 ...
	 * @var string[]
	 */
	private const array COMMON_USERNAMES = [
		'admin',
		'admin1',
		'administrator',
		'root',
	...
	public function blockLoginAttempt( mixed $user, string $username ): mixed {
		...
		if ( in_array( strtolower( $username ), $blocked, true ) ) {
			// Log blocked attempt (no DB query — user_id = 0, username from POST).
			ActivityLog::logEvent( 0, $username, 'blocked_username' );
			...
			return new \WP_Error(
				...
				__( '<strong>Error</strong>: Invalid username or password.', HDA_TEXTDOMAIN )
			);
		}
