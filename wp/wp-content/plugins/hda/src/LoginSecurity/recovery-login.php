...
<form name="otp_validate_form" id="loginform" action="<?php echo esc_url( $args['action'] ?? '' ); ?>" method="post">
	...
	<p class="otp-prompt">
		...
		if ( $channel && $recipientHint ) {
			printf(
				...
				esc_html__( 'A verification code has been sent to your %1$s (%2$s).', HDA_TEXTDOMAIN ),
				'<strong>' . esc_html( $channel ) . '</strong>',
				'<code>' . esc_html( $recipientHint ) . '</code>'
			);
		} else {
			esc_html_e( 'Enter a recovery code.', HDA_TEXTDOMAIN );
		}
		?>
	</p>
	<p class="auth extra">
		<label for="authcode">
			<?php esc_html_e( 'Verification Code:', HDA_TEXTDOMAIN ); ?>
	...
	<p class="otp-help">
		<?php
		$login_opts = LoginSecurity::getOptions();
		$custom_uri = $login_opts[ LoginSecurity::KEY_CUSTOM_LOGIN_URI ] ?? '';
		...
		printf(
			...
			esc_html__( 'No code? %s to resend.', HDA_TEXTDOMAIN ),
			'<a href="' . esc_url( $login_link ) . '">' . esc_html__( 'Login again', HDA_TEXTDOMAIN ) . '</a>'
		);
		?>
	</p>
	<?php submit_button( __( 'Submit', HDA_TEXTDOMAIN ) ); ?>
</form>
