 ...
 *
 * @package HDAddons\LoginSecurity\MagicLink
 * @author  HD
 */

namespace HDAddons\LoginSecurity\MagicLink;
...
final class LoginMagicLink {
	...
	private const string ACTION_MAGIC_LINK = '_magic_link';
	...
	public function interceptLoginPage(): void {
		...
		if ( LoginSecurity::isPostRequest() && ! empty( $_POST['magic_link_request'] ) ) {
		...
		$this->loadForm( [
			...
			'password_login_url'    => $this->getLoginUrl(),
		] );
	}
	...
	private function handleEmailSubmission(): void {
		// Verify CSRF token
		$csrfToken = isset( $_POST['_csrf_token'] ) ? sanitize_text_field( wp_unslash( $_POST['_csrf_token'] ) ) : '';
		if ( empty( $csrfToken ) || ! wp_verify_nonce( $csrfToken, 'magic_link_csrf' ) ) {
			$this->loadForm( [
				'action' => $this->getLoginActionUrl(),
				'error'  => __( 'Security token expired. Please try again.', HDA_TEXTDOMAIN ),
			] );
		...
		if ( empty( $email ) || ! is_email( $email ) ) {
			$this->loadForm( [
				'action' => $this->getLoginActionUrl(),
				'error'  => __( 'Please enter a valid email address.', HDA_TEXTDOMAIN ),
				'email'  => $email,
			] );
		...
		if ( ! $this->isUserEligible( $user ) ) {
			$this->loadForm( [
				'action' => $this->getLoginActionUrl(),
				'error'  => __( 'Your account is not eligible for magic link login. Please use password login.', HDA_TEXTDOMAIN ),
				'email'  => $email,
				'show_password_fallback' => true,
				'password_login_url'     => $this->getLoginUrl(),
			] );
		...
		if ( ! MagicLinkHandler::canRequest( $user->ID ) ) {
			$this->loadForm( [
				'action' => $this->getLoginActionUrl(),
				'error'  => __( 'Too many requests. Please wait a few minutes before trying again.', HDA_TEXTDOMAIN ),
				'email'  => $email,
			] );
		...
		if ( empty( $token ) ) {
			$this->loadForm( [
				'action' => $this->getLoginActionUrl(),
				'error'  => __( 'Failed to generate login link. Please try again.', HDA_TEXTDOMAIN ),
				'email'  => $email,
			] );
		...
		if ( $sent ) {
			...
			if ( class_exists( ActivityLog::class ) ) {
				ActivityLog::logEvent( $user->ID, $user->user_login, 'magic_link_sent' );
			}
		}
	...
	public function handleTokenLogin(): void {
		...
		if ( empty( $token ) ) {
			$this->showTokenError( __( 'Invalid login link.', HDA_TEXTDOMAIN ) );
		...
		if ( ! $row ) {
			$this->showTokenError( __( 'This login link has expired or has already been used. Please request a new one.', HDA_TEXTDOMAIN ) );
		...
		if ( ! $user ) {
			$this->showTokenError( __( 'User account not found.', HDA_TEXTDOMAIN ) );
		...
		if ( ! $this->isUserEligible( $user ) ) {
			...
			$this->showTokenError( __( 'Your account is no longer eligible for magic link login.', HDA_TEXTDOMAIN ) );
		...
		// Log the event
		if ( class_exists( ActivityLog::class ) ) {
			ActivityLog::logEvent( $userId, $user->user_login, 'magic_link_login' );
		}
	...
	private function isEnabled(): bool {
		...
		return ( $opt[ LoginSecurity::KEY_OTP_MODE ] ?? 'disabled' ) === 'magic_link';
	}
	...
	private function getRequiredRoles(): array {
		$opt   = LoginSecurity::getOptions();
		$roles = ! empty( $opt[ LoginSecurity::KEY_OTP_USER_ROLES ] )
			? (array) $opt[ LoginSecurity::KEY_OTP_USER_ROLES ]
	...
	/**
	 * Get the login URL (respects custom login URL).
	 *
	 * @return string
	 */
	private function getLoginUrl(): string {
		$opt       = LoginSecurity::getOptions();
		$customUri = $opt[ LoginSecurity::KEY_CUSTOM_LOGIN_URI ] ?? '';

		if ( $customUri && ! in_array( $customUri, [ 'wp-login.php', 'wp-admin' ], true ) ) {
			return home_url( $customUri );
		}
