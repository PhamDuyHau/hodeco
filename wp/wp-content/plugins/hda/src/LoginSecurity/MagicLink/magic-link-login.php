...
<div id="login_error" class="notice notice-error">
	<p><strong><?php esc_html_e( 'Error:', HDA_TEXTDOMAIN ); ?></strong> <?php echo esc_html( $error ); ?></p>
</div>
<?php endif; ?>
...
<div class="notice notice-success" style="margin-bottom:16px;padding:12px 15px;">
	<p>
		...
		<?php esc_html_e( 'A login link has been sent to your email. Please check your inbox (and spam folder).', HDA_TEXTDOMAIN ); ?>
	</p>
	<p style="font-size:12px;color:#666;margin:4px 0 0;">
		<?php
		printf(
			/* translators: %d: number of minutes */
			esc_html__( 'The link will expire in %d minutes and can only be used once.', HDA_TEXTDOMAIN ),
...
<form name="magic_link_form" id="loginform" action="<?php echo esc_url( $args['action'] ?? '' ); ?>" method="post">
	...
	<p class="otp-prompt">
		<?php esc_html_e( 'Enter your email address and we\'ll send you a link to log in instantly — no password needed.', HDA_TEXTDOMAIN ); ?>
	</p>
	<p>
		<label for="magic_link_email"><?php esc_html_e( 'Email Address', HDA_TEXTDOMAIN ); ?></label>
		<input
			required
			autofocus
			type="email"
	...
	<?php submit_button( __( 'Send Login Link', HDA_TEXTDOMAIN ), 'primary large', 'wp-submit', true ); ?>
</form>
...
<p id="nav" style="text-align:center;margin-top:12px;">
	<a href="<?php echo esc_url( $args['password_login_url'] ?? wp_login_url() ); ?>?force_password=1">
		<?php esc_html_e( '← Login with password', HDA_TEXTDOMAIN ); ?>
	</a>
</p>
<?php endif; ?>
