...
final class MagicLinkHandler {
	...
	// ─── Configuration ────────────────────────────────────
	public const int    TOKEN_EXPIRY  = 300;  // 5 minutes in seconds.
	public const int    MAX_REQUESTS  = 3;    // Max requests per rate window.
	public const int    RATE_WINDOW   = 600;  // Rate limit window (10 min).
	public const int    TOKEN_LENGTH  = 32;   // Random bytes → 64 hex chars.

	// ─── Database ─────────────────────────────────────────
	public const string TABLE_NAME = 'hda_magic_links';
	...
	public static function sendEmail( \WP_User $user, string $loginUrl ): bool {
		...
		$subject = sprintf(
			...
			__( '[%s] Your Magic Login Link', HDA_TEXTDOMAIN ),
			$siteName
		);
