...
final class FileIntegrityAdmin {
	 ...
	 * Menu slug.
	 */
	public const string MENU_SLUG = 'hda-file-integrity';

	/**
	 * Nonce actions.
	 */
	private const string SCAN_NONCE      = 'hda_file_integrity_scan';
	private const string ALLOWLIST_NONCE = 'hda_malware_allowlist';

	/**
	 * Transient keys for cached scan results.
	 */
	private const string CORE_RESULTS_KEY    = 'hda_fi_core_results';
	private const string MALWARE_RESULTS_KEY = 'hda_fi_malware_results';
	...
	public function __construct() {
		...
		add_action( 'admin_enqueue_scripts', static function (): void {
			...
			if ( $handle ) {
				Asset::localize( $handle, 'hdaFileIntegrity', [
					'nonce' => wp_create_nonce( self::SCAN_NONCE ),
					'i18n'  => [
						'scanning'  => __( 'Scanning…', HDA_TEXTDOMAIN ),
						'completed' => __( 'Scan completed', HDA_TEXTDOMAIN ),
						'error'     => __( 'Scan failed. Please try again.', HDA_TEXTDOMAIN ),
					],
				] );
			}
		}, 50 );
	...
	public function addMenuPage(): void {
		add_submenu_page(
			'hda-settings',
			__( 'File Integrity', HDA_TEXTDOMAIN ),
			__( 'File Integrity', HDA_TEXTDOMAIN ),
			Plugin::CAPABILITY,
			self::MENU_SLUG,
			$this->renderPage( ... )
	...
	public static function ajaxCoreScan(): void {
		...
		wp_send_json_success( [
			'message' => __( 'Core integrity scan completed.', HDA_TEXTDOMAIN ),
			'results' => $results,
		] );
	}
	...
	public static function ajaxMalwareScan(): void {
		...
		wp_send_json_success( [
			'message' => __( 'Malware scan completed.', HDA_TEXTDOMAIN ),
			'results' => $results,
		] );
	}
	...
	public static function ajaxVulnScan(): void {
		...
		wp_send_json_success( [
			'message' => __( 'Vulnerability scan completed.', HDA_TEXTDOMAIN ),
			'results' => $results,
		] );
	}
	...
	public function handleAllowlistAction(): void {
		...
		if ( isset( $_POST['allowlist_add'] ) ) {
			$filePath = isset( $_POST['allowlist_file'] ) ? sanitize_text_field( wp_unslash( $_POST['allowlist_file'] ) ) : '';
	...
	public function renderPage(): void {
		...
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'File Integrity', HDA_TEXTDOMAIN ); ?></h1>
			<hr class="wp-header-end">

			<?php if ( isset( $_GET['allowlisted'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'File marked as safe (allowlisted). It will be skipped in future scans.', HDA_TEXTDOMAIN ); ?></p>
				</div>
			<?php endif; ?>

			<?php if ( isset( $_GET['allowlist_removed'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
				<div class="notice notice-info is-dismissible">
					<p><?php esc_html_e( 'File removed from allowlist. It will be scanned again.', HDA_TEXTDOMAIN ); ?></p>
				</div>
			<?php endif; ?>
			...
			<div class="hda-fi-section">
				<div class="card hda-fi-card">
					<div class="hda-fi-card__header">
						<div>
							<h2 class="hda-fi-card__title">🔍 <?php esc_html_e( 'WordPress Core Integrity', HDA_TEXTDOMAIN ); ?></h2>
							<p class="description"><?php esc_html_e( 'Checks WordPress core files against official checksums from WordPress.org.', HDA_TEXTDOMAIN ); ?></p>
						</div>
						<div class="hda-fi-scan-action">
							...
							<button type="button" class="button button-primary hda-fi-scan-btn" data-scan="hda_fi_core_scan">
								<?php esc_html_e( 'Run Core Scan', HDA_TEXTDOMAIN ); ?>
							</button>
						</div>
					</div>
			...
			<div class="hda-fi-section">
				<div class="card hda-fi-card">
					<div class="hda-fi-card__header">
						<div>
							<h2 class="hda-fi-card__title">🛡️ <?php esc_html_e( 'Malware Scanner', HDA_TEXTDOMAIN ); ?></h2>
							<p class="description"><?php esc_html_e( 'Scans plugins, themes, and uploads for known malware signatures and suspicious patterns.', HDA_TEXTDOMAIN ); ?></p>
						</div>
						<div class="hda-fi-scan-action">
							...
							<button type="button" class="button button-primary hda-fi-scan-btn" data-scan="hda_fi_malware_scan">
								<?php esc_html_e( 'Run Malware Scan', HDA_TEXTDOMAIN ); ?>
							</button>
						</div>
					</div>
			...
			<div class="hda-fi-section">
				<div class="card hda-fi-card">
					<div class="hda-fi-card__header">
						<div>
							<h2 class="hda-fi-card__title">🔓 <?php esc_html_e( 'Vulnerability Scanner', HDA_TEXTDOMAIN ); ?></h2>
							<p class="description"><?php esc_html_e( 'Checks installed plugins and themes for known security issues via WordPress.org (outdated, closed, abandoned).', HDA_TEXTDOMAIN ); ?></p>
						</div>
						<div class="hda-fi-scan-action">
							...
							<button type="button" class="button button-primary hda-fi-scan-btn" data-scan="hda_fi_vuln_scan">
								<?php esc_html_e( 'Run Vulnerability Scan', HDA_TEXTDOMAIN ); ?>
							</button>
						</div>
					</div>
	...
	private function renderCoreResults( ?array $results ): void {
		if ( ! $results ) {
			echo '<p class="hda-fi-empty"><em>' . esc_html__( 'No scan results yet. Click "Run Core Scan" to check.', HDA_TEXTDOMAIN ) . '</em></p>';

			return;
		}

		if ( ! empty( $results['error'] ) ) {
			echo '<div class="notice notice-error inline"><p>' . esc_html( $results['error'] ) . '</p></div>';
		...
		printf(
			'<div class="notice %s inline hda-fi-summary"><p>%s <strong>WP %s</strong> — %s files checked — <strong>%d issues</strong> — %s</p></div>',
		...
		// Modified files.
		if ( $modifiedCount > 0 ) {
			echo '<h3 class="hda-fi-heading hda-fi-heading--danger">🔴 ' . esc_html( sprintf( __( 'Modified Files (%d)', HDA_TEXTDOMAIN ), $modifiedCount ) ) . '</h3>';
		...
		// Unknown files.
		if ( $unknownCount > 0 ) {
			echo '<h3 class="hda-fi-heading hda-fi-heading--warning">🟡 ' . esc_html( sprintf( __( 'Unknown Files (%d)', HDA_TEXTDOMAIN ), $unknownCount ) ) . '</h3>';
			echo '<table class="widefat striped hda-fi-table"><thead><tr><th>File</th></tr></thead><tbody>';
		...
		// Missing files.
		if ( $missingCount > 0 ) {
			echo '<h3 class="hda-fi-heading hda-fi-heading--info">🔵 ' . esc_html( sprintf( __( 'Missing Files (%d)', HDA_TEXTDOMAIN ), $missingCount ) ) . '</h3>';
			echo '<table class="widefat striped hda-fi-table"><thead><tr><th>File</th></tr></thead><tbody>';
	...
	private function renderMalwareResults( ?array $results ): void {
		if ( ! $results ) {
			echo '<p class="hda-fi-empty"><em>' . esc_html__( 'No scan results yet. Click "Run Malware Scan" to check.', HDA_TEXTDOMAIN ) . '</em></p>';
		...
		printf(
			'<div class="notice %s inline hda-fi-summary"><p>%s Scanned <strong>%s files</strong> in <strong>%ss</strong> — <strong>%d suspicious files</strong>%s%s%s — %s</p></div>',
			esc_attr( $statusClass ),
			$statusIcon,
		...
		foreach ( $results['findings'] as $finding ) {
			...
			foreach ( $finding['matches'] as $idx => $match ) {
				...
				if ( 0 === $idx ) {
					if ( $isAllowlisted ) {
						$actionCell = sprintf(
							...
							esc_html__( 'Remove from allowlist', HDA_TEXTDOMAIN )
						);
					} else {
						$actionCell = sprintf(
							...
							$pageInput,
							esc_attr( $finding['file'] ),
							esc_html__( 'Mark as safe', HDA_TEXTDOMAIN )
						);
					}
				...
				if ( $isAllowlisted ) {
					$fileLabel .= ' <span class="hda-fi-badge--safe">✅ ' . esc_html__( 'Safe', HDA_TEXTDOMAIN ) . '</span>';
				}
	...
	private function renderVulnResults( ?array $results ): void {
		if ( ! $results ) {
			echo '<p class="hda-fi-empty"><em>' . esc_html__( 'No scan results yet. Click "Run Vulnerability Scan" to check.', HDA_TEXTDOMAIN ) . '</em></p>';
		...
		printf(
			'<div class="notice %s inline hda-fi-summary"><p>%s <strong>%d plugins</strong>, <strong>%d themes</strong> — ' .
		...
		$statusOrder = [ 'vulnerable' => 0, 'closed' => 1, 'abandoned' => 2, 'outdated' => 3, 'not_in_wporg' => 4, 'unknown' => 5, 'up_to_date' => 6 ];
