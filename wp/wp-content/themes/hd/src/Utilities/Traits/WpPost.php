<?php
/**
 * Post, term, and excerpt helper methods.
 *
 * @author HD
 */
...
trait WpPost {
	...
	public static function getTerm( mixed $termId, string $taxonomy = '', string $output = OBJECT ): \WP_Term|\WP_Error|false|array|null {
		...
		// If termId is not numeric, attempt to retrieve the term by slug or name
	 ...
	 *                              Default: 'templates/template-page-home.php'
	 *
	 * @return bool
	 */
	public static function isHomeOrFrontPage( string $homeTemplate = '' ): bool {
		...
		return self::isPageTemplate( $homeTemplate );
	}
	...
	public static function getTaxonomyByPostType( ?string $postType ): ?string {
		...
		return match ( $postType ) {
			'post'    => 'category',
			'product' => self::isWoocommerceActive() ? 'product_cat' : null,
			default   => self::filterSettingOptions( 'post_type_terms' )[ $postType ] ?? null,
		};
	}
	 ...
	 *
	 * @param array $options
	 *     Options for rendering primary term.
	 *
	...
	public static function hashTags( ?string $taxonomy = 'post_tag', int $id = 0, string $sep = '' ): void {
		...
		printf(
			'<div class="hashtag-links links flex items-center flex-wrap gap-3 text-sm font-medium"><span class="text-[#243F40]">%1$s</span><div class="flex flex-wrap gap-2 items-center">%2$s</div></div>',
	 ...
	 *
	 * Delegates to cachedChildTerms() for unified caching strategy.
	 *
	...
	public static function hierarchyTerms(
		...
		// Build tree with runtime options (selected, disabled, etc.)
		$options = [];
	 ...
	 * @param mixed     $selectedRequest Selected term(s).
	 * @param int|null  $disabledParent Parent ID to disable children.
	 * @param bool      $onlyParent     Only return parent term.
	 *
	 * @return array
	 */
	...
	public static function getPageTemplate( string $template ): ?\WP_Post {
		...
		return Cache::remember(
			$cacheKey,
			fn() => self::doPageTemplateQuery( $template ),
			'theme_posts',
			DAY_IN_SECONDS
		);
	}
	...
	public static function cachedTerms( string $taxonomy, array $args = [], int $ttl = HOUR_IN_SECONDS ): array {
		...
		// Build cache key from normalized args
		$cacheKey = 'terms_' . md5( wp_json_encode( $args ) );

		return Cache::remember(
			$cacheKey,
			fn() => self::doCachedTermsQuery( $args ),
			'theme_taxonomies',
			$ttl
		);
	}
	 ...
	 *
	 * @return array Terms or empty array.
	 */
