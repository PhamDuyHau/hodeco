...
trait WpMedia {
	 ...
	 * @return string|false
	 */
	public static function postImageSrc( int|\WP_Post|null $postId = null, string $size = 'thumbnail' ): string|false {
	 ...
	 * @return string|false
	 */
	public static function attachmentImageSrc( mixed $attachmentId, string $size = 'thumbnail' ): string|false {
	 ...
	 * @return string
	 */
	public static function postImageHTML( int|\WP_Post|null $postId = null, string $size = 'post-thumbnail', string|array $attr = '', bool $filter = true ): string {
		$attr = $attr ?: [ 'loading' => 'lazy' ];
	 ...
	 * @return string
	 */
	public static function attachmentImageHTML( mixed $attachmentId, string $size = 'thumbnail', string|array $attr = '', bool $filter = true ): string {
		if ( ! $attachmentId ) {
			return '';
		}
	 ...
	 * @return string
	 */
	public static function iconImageHTML( mixed $attachmentId, string $size = 'thumbnail', string|array $attr = '', bool $filter = false ): string {
		if ( ! $attachmentId ) {
			return '';
		}
	...
	public static function pictureHTML(
		...
		$html               = $cssClass ? '<picture class="' . $cssClass . '">' : '<picture>';
	...
	public static function customSiteLogo( mixed $blogId = 0 ): string {
		...
		if ( $customLogoId ) {
			...
			$isHomepage = self::isHomeOrFrontPage() && ! is_paged();
	...
	public static function siteTitleOrLogo( bool $output = true, ?string $homeHeading = 'h1', ?string $cssClass = 'logo' ): ?string {
		...
		if ( function_exists( 'the_custom_logo' ) && has_custom_logo() ) {
			...
			if ( str_contains( $logo, '<a ' ) ) {
				...
				$html = '<a' . $logoClass . ' title="' . esc_attr( $siteName ) . '" href="' . esc_url( $homeLink ) . '" rel="home">' . $logo . $logoTitle . '</a>';
			}
		} else {
			$html        = '<a' . $logoClass . ' title="' . esc_attr( $siteName ) . '" href="' . $homeLink . '" rel="home">' . esc_html( $siteName ) . $logoTitle . '</a>';
	...
	public static function siteLogo( string $theme = 'default', ?string $cssClass = '' ): string {
		...
		$logoTitle = self::getThemeMod( 'logo_title_setting' );
		$logoTitle = $logoTitle ? '<span class="logo-title">' . esc_html( $logoTitle ) . '</span>' : '';

		$logo = self::attachmentImageHTML( $customLogoId, 'full', $customLogoAttr );

		if ( $cssClass ) {
			return '<div class="' . esc_attr( $cssClass ) . '"><a title="' . esc_attr( $imageAlt ) . '" href="' . esc_url( $homeLink ) . '">' . $logo . $logoTitle . '</a></div>';
		}

		return '<a title="' . esc_attr( $imageAlt ) . '" href="' . esc_url( $homeLink ) . '">' . $logo . $logoTitle . '</a>';
	}
	...
	public static function placeholderSrc( string $cssClass = '', bool $imgWrap = true ): string {
		...
		return '<img loading="lazy" src="' . $src . '" alt="placeholder" class="' . esc_attr( $class ) . '">';
	}
}
