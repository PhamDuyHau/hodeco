import { $ as $$, E as Events, c as createWeakStore, n as nanoid } from "../vendor.We5pwaIa.js";
...
const parseOptions = (el) => {
  var _a, _b;
  const json = (_b = (_a = el == null ? void 0 : el.querySelector(".swiper-wrapper")) == null ? void 0 : _a.dataset) == null ? void 0 : _b.swiperOptions;
  if (!json) return {};
  try {
...
const buildSwiperOptions = (options, classes2, el) => {
  const swiperOptions = {
    loopedSlides: parseInt(options.loopedSlides) || void 0,
...
const getControls = (el) => {
  const wrapper = el.closest(".closest-swiper") || el.parentElement;
  const existing = wrapper == null ? void 0 : wrapper.querySelector(".swiper-controls");
...
const buildNavigation = (options, classes2, controls, fragment) => {
  if (!options.navigation) return null;
  let btnPrev = controls == null ? void 0 : controls.querySelector(".swiper-button-prev");
  let btnNext = controls == null ? void 0 : controls.querySelector(".swiper-button-next");
  if (btnPrev && btnNext) {
    ...
    btnPrev.className = `swiper-button swiper-button-prev ${classes2.prev}`;
    btnNext.className = `swiper-button swiper-button-next ${classes2.next}`;
...
const buildPagination = (options, classes2, controls, fragment) => {
  if (!options.pagination) return null;
  let pagination = controls == null ? void 0 : controls.querySelector(".swiper-pagination");
  if (pagination) {
    pagination.classList.add(classes2.pagination);
  } else {
    pagination = document.createElement("div");
    pagination.className = `swiper-pagination ${classes2.pagination}`;
...
const buildScrollbar = (options, classes2, controls, fragment) => {
  if (!options.scrollbar) return null;
  let scrollbar = controls == null ? void 0 : controls.querySelector(".swiper-scrollbar");
  if (scrollbar) {
    scrollbar.classList.add(classes2.scrollbar);
  } else {
    scrollbar = document.createElement("div");
    scrollbar.className = `swiper-scrollbar ${classes2.scrollbar}`;
...
const buildAutoplayProgress = (options, classes2, controls, fragment) => {
  ...
  return {
    autoplayTimeLeft(s, time, progressValue) {
      const currentSecond = Math.ceil(time / 1e3);
      if (currentSecond !== lastSecond) {
        lastSecond = currentSecond;
        const progressEl = controls == null ? void 0 : controls.querySelector(".swiper-autoplay-progress");
        if (progressEl) {
...
const defaultModules = [Autoplay, Navigation, Pagination, Thumb, freeMode, Grid];
...
const initSwiper = (el) => {
  ...
  if (options.thumbs) {
    ...
    if (thumbsEl) {
      instanceStore.get(thumbsEl) || initSwiper(thumbsEl);
    }
  }
  const swiperOptions = {
    modules: defaultModules,
    ...defaultOptions,
    ...buildSwiperOptions(options, classes2, el)
  };
  const controls = getControls(el);
  const fragment = document.createDocumentFragment();
  const navigation = buildNavigation(options, classes2, controls, fragment);
  if (navigation) swiperOptions.navigation = navigation;
  const pagination = buildPagination(options, classes2, controls, fragment);
  if (pagination) swiperOptions.pagination = pagination;
  const scrollbar = buildScrollbar(options, classes2, controls, fragment);
  if (scrollbar) swiperOptions.scrollbar = scrollbar;
  const progressHandler = buildAutoplayProgress(options, classes2, controls, fragment);
  if (progressHandler) swiperOptions.on = progressHandler;
  if (fragment.childNodes.length) {
    controls.append(fragment);
  }
  const instance = new Swiper(`.${classes2.swiper}`, swiperOptions);
  el.swiper = instance;
...
//# sourceMappingURL=fx-slider.CL-AExEo.js.map
