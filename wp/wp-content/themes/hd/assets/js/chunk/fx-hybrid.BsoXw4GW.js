...
for (let i = 0; i < SIN_CACHE_SIZE; i++) {
  sinCache[i] = Math.sin(i) * 1e4;
}
const fastSeededRandom = (seed) => {
  const idx = (seed | 0) & 255;
  const x = sinCache[idx] + seed * 1e-3;
...
class HybridLayout {
  constructor(containerWidth, opts = {}) {
    ...
    this.heightVariation = opts.heightVariation || 0.4;
    this.randomSeed = opts.randomSeed || Date.now();
    this.featureChance = opts.featureChance || 0.15;
    this.stagger = opts.stagger || false;
    this.staggerAmount = opts.staggerAmount || 0.02;
  }
  calculate(items) {
    const len = items.length;
    if (!len) return { boxes: [], containerHeight: 0 };
    const boxes = new Array(len);
    let boxCount = 0;
    let top = 0;
    let rowIndex = 0;
    ...
    for (let i = 0; i < len; i++) {
      let ar = items[i].width / items[i].height;
      if (featured[i]) ar *= 1.3;
      aspectRatios[i] = ar;
    }
    for (let i = 0; i < len; i++) {
  ...
  _addRowToBoxesOptimized(boxes, boxCount, items, aspectRatios, featured, startIdx, endIdx, height, top, staggerOffset, rowIndex, gutter, containerWidth, seed) {
    ...
    for (let i = startIdx; i <= endIdx; i++) {
      ...
      const item = items[i];
      const isFeatured = featured[i] === 1;
      let width = aspectRatios[i] * scale;
    ...
    if (boxCount > 0 && rowLen > 0) {
      const lastBox = boxes[boxCount - 1];
  ...
  // Keep old method for compatibility but mark as legacy
  _addRowToBoxes(boxes, row, height, top, staggerOffset, rowIndex) {
    let left = staggerOffset;
    const rowWidth = this.containerWidth - this.gutter * (row.length - 1);
    let totalAR = 0;
    for (let i = 0; i < row.length; i++) totalAR += row[i].ar;
    const scale = rowWidth / totalAR;
    ...
    for (let i = 0; i < rowLen; i++) {
      const item = row[i];
      let width = item.ar * scale;
      if (i > 0 && i < rowLen - 1) {
        width *= 1 + (fastSeededRandom(seed + item.index * 17 + rowIndex) - 0.5) * 0.04;
      }
      const verticalNudge = i > 0 && i < rowLen - 1 ? (fastSeededRandom(seed + item.index * 23) - 0.5) * (gutter * 0.3) : 0;
      boxes.push({
        left: left > 0 ? left : 0,
        top: top + verticalNudge,
        width,
        height: item.featured ? height * 1.05 : height,
        element: item.element,
    ...
    if (boxes.length > 0 && rowLen > 0) {
      const lastBox = boxes[boxes.length - 1];
      const overflow = lastBox.left + lastBox.width - this.containerWidth;
      if (overflow > 1 || overflow < -1) {
        lastBox.width -= overflow;
      }
    }
...
class FxHybrid {
  constructor(el, opts = {}) {
    ...
    this.opts = {
      ...
      staggerAmount: 0.015,
      randomSeed: null,
      // null = use Date.now() for different layouts each time
  ...
  getBaseHeight() {
    const w = innerWidth, h = this.opts.baseHeight;
    return h.lg && w >= 1024 && h.lg || h.md && w >= 768 && h.md || h.default || 200;
  }
  layout() {
    ...
    boxes.forEach((box, i) => {
      ...
      if (box.featured) {
        box.element.classList.add("hybrid-item--featured");
      } else {
        box.element.classList.remove("hybrid-item--featured");
      }
    });
...
export {
  FxHybridAPI as default
};
//# sourceMappingURL=fx-hybrid.BsoXw4GW.js.map
