...
// Seeded random for consistent layouts - optimized inline version
...
for (let i = 0; i < SIN_CACHE_SIZE; i++) {
	sinCache[i] = Math.sin(i) * 10000;
}

const fastSeededRandom = (seed) => {
	const idx = (seed | 0) & 0xff;
	const x = sinCache[idx] + seed * 0.001;
	return x - Math.floor(x);
};
...
class HybridLayout {
	...
	_addRowToBoxes(boxes, row, height, top, staggerOffset, rowIndex) {
		...
		for (let i = 0; i < rowLen; i++) {
			...
			// Micro-variation for middle items
			if (i > 0 && i < rowLen - 1) {
